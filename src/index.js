...
const inputSearch = document.getElementById('location-search');
...
const icon = new Map()
  ...
  .set(
    '10n',
    `<svg id="&#x56FE;&#x5C42;_1" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="qi-351" viewBox="0 0 16 16">
  <defs>
    <style>
  ...
  .set(
    '13d',
    `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="qi-514" viewBox="0 0 16 16">
...
async function getWeatherAPI() {
  ...
  cloudy.innerHTML = await `${data.cloud}%`;
  humidity.innerHTML = await `${data.humidity}%`;
  wind.innerHTML = await `${data.wind_speed}m/s`;
  rain.innerHTML = await `${data.rain}%`;
  weatherDescription.innerHTML = await `${data.description}`;
...
window.onload(document.querySelector('search-btn'), getWeatherAPI());
